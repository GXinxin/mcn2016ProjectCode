function [px, py, id] = plot_tSNE_wholeBrain_GUI2(corrBetween, corrId, isArrow, p, d, s, pts_tsne, movie, labels, tmps, stateId_embed, states, perVector)
% p: perplexity value
...
%         scatter(pts(:, 1), pts(:, 2), 10*(corrBetween - min(corrBetween) + 0.001), colorMat, 'markerfacecolor', 'flat'); hold on
        if isArrow
            quiver(pts(1:NA-1, 1), pts(1:NA-1, 2), pts(2:NA, 1) - pts(1:NA-1, 1), pts(2:NA, 2) - pts(1:NA-1, 2), 0); hold on
        end
        scatter(pts(corrId, 1), pts(corrId, 2), 3*ones(length(corrId), 1), 'k'); hold on
    else
...
%         scatter3(pts(corrId, 1), pts(corrId, 2), pts(corrId, 3), 3*ones(length(corrId), 1), 'k', 'markerfacecolor', 'flat'); hold on
    end
...
%     id = id(1);
    ...
    figure;
    subplot(2, 1, 1); imagesc(movie(:, :, id(i))); axis equal; axis off; caxis([-0.1, 0.4]); colorbar
